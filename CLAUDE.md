...
An interactive web page for an eight-year-old, and for sharing, that makes 4.6 billion years feel long. One LEGO brick for every year, stacked back in time as they scroll. Scrolling forward builds the stack and scrolling back undoes it, instantly and in proportion to how far they scroll. The stack passes things a child knows (a door, their house, the Eiffel Tower, airplanes, the space station) on its way to wrapping around the Earth, compacting ten bricks into one whenever it would otherwise outgrow the screen so it always stays visible. At most one popup is open per side at a time — the latest time event on the right, the latest thing they've passed on the left — and every older fact collapses to a small pin on its brick that reopens on tap; every fact they've reached stays in a scrapbook even after they scroll back past it. Controls live behind one compact menu tab in the corner. The feeling comes from cost: a vigorous, sustained scroll takes about three minutes of hard scrolling to finish, and a curious one takes much longer — that's the lesson. Every ratio on screen is real. Icons are labels, not to scale.
...
- Facts (ages, heights) live in `src/lib/landmarks.ts` and `src/lib/beats.ts` with a source comment. Do not invent facts. If a number is uncertain, round and say "about". Copy rules for a time event's title/line, and the popup/pin/scrapbook lifecycle, are in `docs/principles.md`'s "Facts" section.
