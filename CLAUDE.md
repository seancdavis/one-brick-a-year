...
- Facts (ages, heights) live in `src/lib/landmarks.ts` and `src/lib/beats.ts` with a source comment. Do not invent facts. If a number is uncertain, round and say "about". Copy rules for a time event's title/line, and the tag/scrapbook lifecycle, are in `docs/principles.md`'s "Facts" section.
